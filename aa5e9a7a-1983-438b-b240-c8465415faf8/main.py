...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO UNIVERSE (SERIES J - SWING RUNNER) ---
        # Goal: Restore High Returns for Small Account by HOLDING WINNERS.
        ...
        # LOCKOUT: Still 1 Day to protect Cash Account rules.
        ...
        # REMOVED: Rotation Stages (No scaling out. All In, All Out.)
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            try:
                log(f"SERIES J (SWING): No Profit Cap. Letting Winners Run.")
            except:
                pass
        ...
        if is_defensive:
            ...
            if p_hist:
                ...
                # --- EXIT LOGIC (SIMPLIFIED FOR SWING) ---
                
                # 1. STOP LOSS (4.0x ATR) -> EMERGENCY EXIT -> LOCKOUT
                if current_price <= self.entry_price - (4.0 * atr):
                    log(f"STOP: 4.0x ATR Hit on {self.primary_asset}. Locking 1 Day.")
                    self.system_lockout_counter = self.lockout_duration
                    self.primary_asset = None
                    return TargetAllocation({"SGOV": 1.0}) 

                # 2. TRAILING STOP (5.0x ATR) -> TREND BENT -> LOCKOUT
                # We removed the +20% Cap. We only sell if the trend reverses.
                if current_price <= self.peak_price - (5.0 * atr):
                     log(f"EXIT: Trailing Stop Hit on {self.primary_asset}. Banking & Locking.")
                     self.system_lockout_counter = self.lockout_duration
                     self.primary_asset = None
                     return TargetAllocation({"SGOV": 1.0})

                # 3. HOLD
                # If neither stop is hit, we DO NOTHING. We hold overnight.
