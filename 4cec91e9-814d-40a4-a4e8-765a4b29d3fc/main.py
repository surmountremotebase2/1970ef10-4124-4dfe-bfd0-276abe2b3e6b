...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO UNIVERSE (SERIES E) ---
        # 1. Back to the "Winning Signal" (Fast response)
        ...
        # *** RESTORED: 40-BAR MOMENTUM ***
        # We learned that 390 (Weekly) is too slow for 3x ETFs. 
        # We need to catch the move early (3-4 hours).
        self.mom_len = 40      
        
        self.safe_len = 20     
        self.atr_period = 14
        
        # Cooldown: Reduced to 6 bars (30 mins) because we have wider stops now.
        # If we get stopped out at 4x ATR, it's a real crash, so 30 mins is enough wait.
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            try:
                log(f"SERIES E (WIDE STANCE): Stop=4.0x ATR | Mom={self.mom_len}")
            except:
                pass
        ...
        if is_defensive:
            ...
            if p_hist:
                ...
                # *** THE FIX: WIDER STOPS ***
                # Changed from 2.5x -> 4.0x ATR
                # 3x Leveraged ETFs need this room to survive daily noise.
