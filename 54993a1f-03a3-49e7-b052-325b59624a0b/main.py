...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO UNIVERSE (SERIES L - THE RATCHET) ---
        # Goal: Seed Money Growth. 
        # Tweak: Dynamic Trailing Stop to squeeze more profit from winners.
        ...
        # LOCKOUT: 1 Day (Strict Compliance)
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            try:
                log(f"SERIES L (RATCHET): Trail 8.0x -> Tightens to 4.0x after +15% gain.")
            except:
                pass
        ...
        if is_defensive:
            ...
            if p_hist:
                ...
                # --- EXIT LOGIC (THE RATCHET) ---
                
                # 1. STOP LOSS (6.0x ATR) -> SURVIVAL MODE
                ...
                # 2. DYNAMIC TRAILING STOP (THE RATCHET)
                # SQUEEZE: If Profit > 15%, Tighten Trail to 4.0x.
                # NORMAL: If Profit < 15%, Keep Trail Wide at 8.0x.
                
                trail_mult = 8.0
                if profit_pct > 0.15:
                    trail_mult = 4.0
                
                if current_price <= self.peak_price - (trail_mult * atr):
                     log(f"EXIT: Trailing Stop ({trail_mult}x) Hit on {self.primary_asset}. Profit: {profit_pct:.2%}. Locking.")
                     self.system_lockout_counter = self.lockout_duration
                     self.primary_asset = None
                     return TargetAllocation({"SGOV": 1.0})
