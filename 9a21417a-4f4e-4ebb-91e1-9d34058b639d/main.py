...
class TradingStrategy(Strategy):
    def __init__(self):
        # Final 2026 Macro Roster (UPRO removed to reduce server load)
        ...
        self.take_profit_pct = 0.08 # <-- Adjusted to 8%
    ...
    def run(self, data):
        ...
        # --- 1. SWING MANAGEMENT (8% Take-Profit & 8% Trailing Stop) ---
        if self.active_trade:
            ...
            # OFFENSIVE EXIT: Lock in 8% hard gain
            if self.entry_price and cp >= self.entry_price * (1 + self.take_profit_pct):
                log(f"TAKE PROFIT: {self.active_ticker} exit at {cp}. Secured 8% gain.")
                self.active_trade = False
                self.active_ticker = None
                self.peak_price = None
                self.entry_price = None
