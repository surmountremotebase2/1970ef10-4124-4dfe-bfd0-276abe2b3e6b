...
class TradingStrategy(Strategy):
    def __init__(self):
        ...
        # OFFICIAL PHASE 2 VERSION
        # Objective: Max Growth for >$25k accounts (Margin Account).
        # Roster: Nitro 7 (Same universe, faster execution).
        ...
        # MARKET GOVERNOR: 5 Days (390 bars)
        # Standard trend filter for margin trading.
        self.trend_len = 390   
        
        # LOCKOUT: 0 (Instant Re-Entry)
        # Margin accounts do not need settlement delays.
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            log(f"NITRO SERIES F: MACHINE GUN MODE. Lockout: DISABLED.")
            self.debug_printed = True

        # 1. GOVERNOR CHECK (5-Day Trend)
        ...
        if p_hist:
            ...
            # Tighter stop for Series F because we can re-enter instantly.
            if curr <= self.entry_price - (4.0 * atr) or curr <= self.peak_price - (8.0 * atr):
                log(f"EXIT: {self.primary_asset} Trend Broken. Instant Re-Entry Available.")
                self.primary_asset = None
                return TargetAllocation({"SGOV": 1.0})
