...
# Establish the mathematical take profit trigger
...
state_changed = True
continue # Exit managed, skip the stop check
...
# --- TIME SHIELD innovate Addition ---
# Block the engine from scanning entries between 9:30 AM and 9:55 AM (US EST)
# This lets the opening bell volume spikes absorb and normalize, preventing false RVOL triggers.
...
# Phase 3: DynamicRemainder Execution (Capped at exactly remaining margin)
if candidates:
...
# Important: For entry, we still use the 'close' price as the simulation purchase price.
entry_px = df['close'].iloc[-1]
self.entry_prices[ticker] = entry_px
# Initialize the HWM to the high of the same bar we bought the close of.
