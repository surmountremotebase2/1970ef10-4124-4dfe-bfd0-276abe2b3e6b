from surmount.base_class import Strategy, TargetAllocation
from surmount.logging import log
import pandas as pd
import numpy as np

class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO "SEED MONEY" OPTIMIZED ---
        
        # 1. CORRELATED OFFENSE (Subject to SPY Governor)
        # Removed FNGU (Redundant with SOXL)
        self.equity_tickers = ["SOXL", "DFEN"] 
        
        # 2. NON-CORRELATED OFFENSE (Immune to SPY Governor)
        # Removed URNM (Too slow)
        self.uncorrelated_tickers = ["BITU", "UCO", "SILJ"]
        
        self.tickers = self.equity_tickers + self.uncorrelated_tickers
        self.safety = ["SGOV"]
        self.vixy = "VXX"
        self.spy = "SPY"

        # --- PARAMETERS ---
        self.vix_ma_len = 390 # 5 Days
        self.mom_len = 40 # Momentum Window
        self.trend_len = 156 # SPY Trend (2 Days)
        self.lockout_duration = 39
        self.atr_period = 14
        
        self.system_lockout_counter = 0
        self.primary_asset = None
        self.entry_price = None
        self.peak_price = None
        self.debug_printed = False

    @property
    def interval(self):
        return "5min"

    @property
    def assets(self):
        return self.tickers + self.safety + [self.vixy, self.spy]

    def get_history(self, d, ticker):
        history = []
        for bar in d:
            if ticker in bar:
                history.append(bar[ticker])
        return history

    def calculate_atr(self, ticker_data):
        if not ticker_data: return 0
        df = pd.DataFrame(ticker_data)
        tr = pd.concat([df['high'] - df['low'],
                        np.abs(df['high'] - df['close'].shift()),
                        np.abs(df['low'] - df['close'].shift())], axis=1).max(axis=1)
        return tr.rolling(window=self.atr_period).mean().iloc[-1]

    def calculate_momentum(self, history, length):
        if len(history) >= length:
            return (history[-1]["close"] / history[-length]["close"]) - 1
        return -999

    def run(self, data):
        d = data["ohlcv"]
        if not d: return None
        
        if not self.debug_printed:
            log(f"NITRO SEED MONEY: LIVE. Asset Split: Equity vs Uncorrelated.")
            self.debug_printed = True

        # 1. LOCKOUT CHECK
        if self.system_lockout_counter > 0:
            self.system_lockout_counter -= 1
            return TargetAllocation({"SGOV": 1.0})

        # 2. GLOBAL DEFENSE (VXX SHIELD)
        # Added 1% Buffer to prevent whipsaw
        vix_data = self.get_history(d, self.vixy)
        if len(vix_data) >= self.vix_ma_len:
            vix_ma = sum([x["close"] for x in vix_data[-self.vix_ma_len:]]) / self.vix_ma_len
            if vix_data[-1]["close"] > (vix_ma * 1.01):
                self.primary_asset = None
                return TargetAllocation({"SGOV": 1.0})

        # 3. SPY GOVERNOR STATUS
        # We check SPY, but we don't return SGOV immediately.
        spy_hist = self.get_history(d, self.spy)
        spy_bullish = False
        if self.calculate_momentum(spy_hist, self.trend_len) > 0:
            spy_bullish = True

        # 4. SCORING & SELECTION
        scores = {}
        for t in self.tickers:
            mom = self.calculate_momentum(self.get_history(d, t), self.mom_len)
            
            # LOGIC FIX:
            # If SPY is Bearish, FORCE score of Equity assets (SOXL, DFEN) to -999
            # This allows BITU/UCO to still win if they are rallying.
            if not spy_bullish and t in self.equity_tickers:
                scores[t] = -999
            else:
                scores[t] = mom

        leader = sorted(scores, key=scores.get, reverse=True)[0]
        leader_score = scores[leader]

        # 5. EXECUTION LOGIC
        
        # A. ENTRY
        if self.primary_asset is None:
            if leader_score > 0:
                self.primary_asset = leader
                self.entry_price = self.get_history(d, leader)[-1]["close"]
                self.peak_price = self.entry_price
                log(f"ENTRY: {leader} at {self.entry_price}. SPY Bullish: {spy_bullish}")
                return TargetAllocation({leader: 1.0})
            else:
                return TargetAllocation({"SGOV": 1.0})

        # B. MANAGEMENT
        p_hist = self.get_history(d, self.primary_asset)
        if p_hist:
            curr = p_hist[-1]["close"]
            self.peak_price = max(self.peak_price, curr)
            atr = self.calculate_atr(p_hist) or (curr * 0.02)
            
            # STOP LOSS (4.5x) or TRAILING STOP (8.0x)
            if curr <= self.entry_price - (4.5 * atr) or curr <= self.peak_price - (8.0 * atr):
                log(f"EXIT: {self.primary_asset} Stop Hit.")
                self.system_lockout_counter = self.lockout_duration
                self.primary_asset = None
                return TargetAllocation({"SGOV": 1.0})

            return TargetAllocation({self.primary_asset: 1.0})
            
        return None