...
class TradingStrategy(Strategy):
    def __init__(self):
        ...
        # ACTION: Bifurcated Equities vs. Alternatives to prevent SPY from blocking non-correlated assets.
        # ACTION: Lengthened momentum lookback and added VXX confirmation to reduce whipsaw and protect cash settlement (T+1).
        
        # Bifurcated Asset Lists
        ...
        self.vixy = "VXX" # Fixed ghost workaround prompt applied
        self.spy = "SPY"

        # --- PARAMETERS ---
        self.vix_ma_len = 390 # 5 Days (390 * 5min)
        self.mom_len = 78 # Momentum Window increased to 1 Full Day (78 * 5min)
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            log(f"NITRO K: Bifurcated Governor Active. T+1 Cash Account Parameters Set.")
            self.debug_printed = True

        # 1. LOCKOUT CHECK (Churn Protection for Cash Accounts)
        if self.system_lockout_counter > 0:
            self.system_lockout_counter -= 1
            return TargetAllocation({"SGOV": 1.0})

        # 2. VXX SHIELD (Hard Defense with 3-Candle Confirmation & Lockout)
        ...
        if len(vix_data) >= self.vix_ma_len:
            ...
            # Require 3 consecutive 5-min closes above the MA to prevent whipsaw
        ...
        # 3. SPY GOVERNOR CHECK (Evaluate Trend)
        ...
        if self.primary_asset is None:
            if scores[leader] > 0:
                # Apply Governor ONLY to Equities
