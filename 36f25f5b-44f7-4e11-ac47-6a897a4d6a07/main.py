...
class TradingStrategy(Strategy):
    ...
    def run(self, data):
        ...
        # --- FIX #1: Robust Holdings Fetcher ---
        # We try 'holdings' first, then 'positions'.
        raw_positions = data.get("holdings", data.get("positions", {}))
        
        # --- FIX #2: Robust Quantity Check (The TypeError Fix) ---
        # We define a helper to get quantity safely, whether it's a dict or a float.
        def get_quantity(ticker):
            if ticker not in raw_positions: return 0.0
            val = raw_positions[ticker]
            # If it's a dictionary (e.g. {"quantity": 10}), extract it.
            if isinstance(val, dict):
                return val.get("quantity", 0.0)
            # If it's just a number (e.g. 10.0), return it directly.
            return float(val)
        ...
        for ticker in self.offensive_assets:
            ...
            if qty > 0:
                # FIX #3: Safe Price Access
                # Check if we have data for this ticker in the latest bar
        ...
        # --- FIX #4: Correct History Parsing (The Symbol KeyError Fix) ---
        # We parse assuming structure is list of {ticker: {data}}
