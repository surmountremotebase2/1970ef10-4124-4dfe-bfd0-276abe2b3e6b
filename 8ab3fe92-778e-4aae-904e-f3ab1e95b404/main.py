...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO UNIVERSE (SERIES K - 2026 TARGETS) ---
        # "THE IRON GRIP"
        # Objective: Grow $10k -> $25k while complying with Cash/PDT rules.
        # Logic: Swing Trading with wide stops to prevent premature lockouts.
        
        # THE ACTUAL 7 TARGET INVESTMENTS FOR 2026:
        self.tickers = [
            "SOXL", # Semis 3x
            "FNGU", # Big Tech 3x
            "DFEN", # Defense 3x
        ...
        # COMPLIANCE LOCKOUT: 1 Full Trading Day (78 bars)
        # Prevents Cash Settlement violations and PDT flags.
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            try:
                log(f"NITRO SERIES K: Validated. Targets: {self.tickers}")
            except:
                pass
            self.debug_printed = True

        # --- SYSTEM LOCKOUT CHECK ---
        # If we sold yesterday, we are locked out until today's settlement.
        ...
        if is_defensive:
            ...
            if p_hist:
                ...
                # --- EXIT LOGIC (WIDE STANCE) ---
                
                # 1. STOP LOSS (6.0x ATR) -> EMERGENCY EXIT
                ...
                # 2. TRAILING STOP (8.0x ATR) -> TREND BREAK
                ...
                # 3. HOLD
                # We stay in until the trend truly dies.
