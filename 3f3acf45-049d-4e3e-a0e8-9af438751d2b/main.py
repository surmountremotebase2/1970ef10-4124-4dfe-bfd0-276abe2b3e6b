...
class TradingStrategy(Strategy):
    ...
    def run(self, data):
        ...
        # --- FIX: USE 'holdings' INSTEAD OF 'positions' ---
        # The backtester uses "holdings". We use .get() to be safe.
        ...
        for ticker in self.offensive_assets:
            if ticker in current_positions and current_positions[ticker]["quantity"] > 0:
                ...
                # Note: Need at least 14 bars for ATR
                if len(ohlcv) > 20:
                    try:
                        atr_value = ATR(ticker, ohlcv, 14)[-1]
                    except:
                        atr_value = current_price * 0.05 # Fallback if ATR fails
                else:
                    atr_value = current_price * 0.05 # Fallback for early data
        ...
        top_assets = sorted(scores, key=scores.get,