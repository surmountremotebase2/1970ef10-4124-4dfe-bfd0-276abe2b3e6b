...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO SERIES K-OPTIMIZED ---
        # Goal: Faster $10k -> $25k growth.
        # Strategy: Increased aggression in trend-following with faster re-entry.
        ...
        # *** OPTIMIZATION: Faster Market Context (2 Days) ***
        self.trend_len = 156   
        
        # *** OPTIMIZATION: Half-Day Lockout (39 bars) ***
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            log(f"SERIES K-OPTIMIZED: Aggressive Regime | Lockout: 3.5 hrs")
        ...
        if p_hist:
            ...
            # *** OPTIMIZATION: Tighter 4.5x ATR Stop ***
            if curr <= self.entry_price - (4.5 * atr) or curr <= self.peak_price - (8.0 * atr):
                log(f"EXIT: {self.primary_asset} Trend Broken. Lockdown engaged.")
