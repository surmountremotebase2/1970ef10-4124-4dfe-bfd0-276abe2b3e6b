...
class TradingStrategy(Strategy):
    def __init__(self):
        # 2026 All-Weather Basket
        self.tickers = ["TECL", "DFEN", "TNA", "GDXU"]
        
        # Parameters
        self.vwap_len = 12 
        self.rvol_threshold = 1.8 
        self.trailing_stop_pct = 0.045 # 4.5% Buffer
        self.max_allocation = 0.50 # Using 50% to allow for T+1 rotation
    ...
    def get_conviction_score(self, history):
        ...
        # Macro Trend Check (Above 1-day/78-bar SMA)
    ...
    def run(self, data):
        ...
        # --- 1. ACTIVE SWING MANAGEMENT (No EOD Sell-off) ---
        if self.active_trade:
            ...
            if cp <= self.peak_price * (1 - self.trailing_stop_pct):
                log(f"SWING STOP: {self.active_ticker} hit trailing stop at {cp}.")
                self.active_trade = False
                self.active_ticker = None
                self.peak_price = None
                return TargetAllocation({})
            
            # Continue holding the position overnight
            return None

        # --- 2. PREDATORY SELECTION SCAN ---
        scores = {}
        ...
        if scores:
            ...
            log(f"SWING ENTRY: {best_ticker} | RVOL Score: {scores[best_ticker]:.2f}")
