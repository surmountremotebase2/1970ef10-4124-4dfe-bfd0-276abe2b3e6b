...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO UNIVERSE (SERIES G - SMALL ACCOUNT) ---
        ...
        # --- PARAMETERS ---
        self.vix_ma_len = 390  
        self.mom_len = 40 # Fast Entry
        self.trend_len = 390 # Market Trend
        self.safe_len = 20     
        self.atr_period = 14
        
        # *** THE BRIDGE FIX ***
        # Changed Cooldown from 6 (30 mins) to 78 (1 Full Trading Day).
        # This prevents "Churn" and keeps you safe from PDT/Cash Settlement violations.
        self.cooldown_bars = 78 
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            try:
                log(f"SERIES G (BRIDGE): Cooldown set to 1 Day (78 bars)")
            except:
                pass
        ...
        if is_defensive:
            ...
            if p_hist:
                ...
                if current_price <= self.entry_price - (4.0 * atr):
                    log(f"STOP: 4.0x ATR Hit on {self.primary_asset}. Cooling down for 1 Day.")
