...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO UNIVERSE (SERIES I - STRICT CASH COMPLIANCE) ---
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            try:
                log(f"SERIES I (STRICT): Locking on Wins AND Losses")
            except:
                pass
        ...
        if is_defensive:
            ...
            if p_hist:
                ...
                # STOP LOSS (4.0x ATR) -> LOCKDOWN
                if current_price <= self.entry_price - (4.0 * atr):
                    log(f"STOP: 4.0x ATR Hit on {self.primary_asset}. Locking for 1 Day.")
                    self.system_lockout_counter = self.lockout_duration
                    self.primary_asset = None
                    self.rotation_stage = 0
                    return TargetAllocation({"SGOV": 1.0}) 

                # ROTATION (WIN) -> LOCKDOWN (New Rule for Series I)
                # We cannot rotate same-day in cash account. We must take profit and wait.
                if self.rotation_stage == 0 and profit_pct >= 0.20:
                    log(f"WIN: +20% Gain on {self.primary_asset}. Banking Profit & Locking 1 Day.")
                    self.system_lockout_counter = self.lockout_duration
                    self.primary_asset = None
                    self.rotation_stage = 0
                    return TargetAllocation({"SGOV": 1.0})
                
                # TRAILING STOP (WIN) -> LOCKDOWN
                elif self.rotation_stage == 1:
                     if current_price <= self.peak_price - (5.0 * atr):
                        log(f"WIN: Trail Hit on {self.primary_asset}. Banking Profit & Locking 1 Day.")
                        self.system_lockout_counter = self.lockout_duration
                        self.primary_asset = None
                        self.rotation_stage = 0
                        return TargetAllocation({"SGOV": 1.0})
