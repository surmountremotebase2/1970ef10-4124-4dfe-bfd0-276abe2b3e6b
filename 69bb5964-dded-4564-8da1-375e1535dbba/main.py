...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO UNIVERSE (SERIES D) ---
        # Same universe, but now we trade the WEEKLY trend, not the HOURLY.
        ...
        # *** THE SERIES D PIVOT ***
        # Changed from 40 to 390. 
        # 390 bars * 5 mins = 1 Full Trading Week (approx).
        self.mom_len = 390      
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            try:
                log(f"SERIES D (SWING): Lookback -> {self.mom_len} bars")
            except:
                pass
        ...
        if is_defensive:
            ...
            # Only buy if the 5-day trend is actually positive
            if scores[leader] < 0:
