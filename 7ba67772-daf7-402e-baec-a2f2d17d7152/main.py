...
class TradingStrategy(Strategy):
    def __init__(self):
        # --- NITRO UNIVERSE (SERIES C) ---
        # Added BITU (Bitcoin) for diversification so we aren't just reliant on Tech/Energy.
        # SILJ is included, but the "Negative Momentum Filter" will block it if it's crashing.
        ...
        self.safe_len = 20     
        self.atr_period = 14
        self.cooldown_bars = 12 # 1 Hour Cooldown
    ...
    def run(self, data):
        ...
        if not self.debug_printed:
            try:
                log(f"SERIES C: Asset List -> {self.tickers}")
            except:
                pass
            self.debug_printed = True

        # 2. VXX REGIME CHECK (Shield)
        ...
        if is_defensive:
            ...
            # OFFENSIVE MODE (With Absolute Momentum Check)
            scores = {}
            ...
            # --- THE FIX: ABSOLUTE MOMENTUM FILTER ---
            # If the "Best" asset is actually negative (losing money), 
            # DO NOT BUY. Go to Safety (SGOV) instead.
            if scores[leader] < 0:
                if self.primary_asset is None:
                    # Log only occasionally to avoid spam, or just return SGOV
                    return TargetAllocation({"SGOV": 1.0})
                
                # If we hold a position but the leader is now negative, 
                # we let the Trailing Stop/Stop Loss handle the exit below.
            ...
            if self.primary_asset is None:
                ...
                if current_bar_index - last_stop < self.cooldown_bars:
                    # COOLDOWN ACTIVE: Park in SGOV
                    return TargetAllocation({"SGOV": 1.0})
                
                # EXECUTE ENTRY (Only if momentum is positive)
            ...
            if p_hist:
                ...
                if current_price <= self.entry_price - (2.5 * atr):
                    log(f"STOP: 2.5x ATR Hit on {self.primary_asset}. Cooling down in SGOV.")
                    self.last_stop_index[self.primary_asset] = current_bar_index
                    self.primary_asset = None
                    self.rotation_stage = 0
                    # FIX: Return SGOV instead of empty dict
                    return TargetAllocation({"SGOV": 1.0}) 
