...
class TradingStrategy(Strategy):
    def __init__(self):
        # Macro Roster: Equities, Commodities, Bitcoin, and Volatility Hedge
        self.tickers = ["TECL", "GDXU", "SOXL", "UPRO", "UCO", "AGQ", "IBIT", "VIXY"]
    ...
    def run(self, data):
        ...
        if self.active_trade:
            ...
            # BINARY EXIT: 100% exit at 10% hard gain
            if self.entry_price and cp >= self.entry_price * (1 + self.take_profit_pct):
                log(f"TARGET HIT: {self.active_ticker} secured 10% gain at {cp}.")
                self.active_trade = False
                self.active_ticker = None
                self.peak_price = None
                self.entry_price = None
            ...
            if cp <= self.peak_price * (1 - self.trailing_stop_pct):
                log(f"STOP TRIGGERED: {self.active_ticker} exit at {cp}. Peak was {self.peak_price}.")
                self.active_trade = False
                self.active_ticker = None
                self.peak_price = None
                self.entry_price = None
        ...
        if scores:
            ...
            log(f"ENTRY: {best_ticker} | RVOL: {scores[best_ticker]:.2f} | Entry: {self.entry_price}")
